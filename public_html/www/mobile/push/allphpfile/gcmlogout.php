...
include_once $my_path.'class/my_mysql.php';      //자식 클래스
	...
	$rr = mysql_query("update Anyting_gcmid set login='no', bell = 1, endtime = '".$aday."' where gcmid = '".$regid."' limit 1",$rs);
	...
	$rr = mysql_query($ss, $rs);
		...
		//중복된 내용은 삭제 한다.
		$aa = "select id, memid, udid, project,  gcmid from Anyting_gcmid where udid <> 'udid None' order by udid, id desc";
		...
		while($row = mysql_fetch_array($rr1)){
			if($row[udid] != $fndudid){  //새로운 폰의 정보가 전달 되었다.
				...
				if(in_array($row[gcmid], $gcmarray)){  //배열에 같은 값이 있다.
					$ss1 = "delete from Anyting_gcmid where id = ".$row[id]." and udid = '".$fndudid."'   limit 1 ";
