...
class Member extends MySQL
{
	...
	function dblMemIdDel($memid, $proje){
		...
		$su = parent::get_count("Anyting_gcmid", " memid = '".$memid."' and project = '".$proje."' ");
		if($su > 1){
			$rs = parent::query("select * from Anyting_gcmid where memid = '".$memid."' and project = '".$proje."' order by endtime desc");
			$c = 0;
			while($row = mysql_fetch_array($rs)){
				if($c > 0){
					parent::bas_delete("Anyting_gcmid", " id = ".$row[id]." limit 1");
				}
