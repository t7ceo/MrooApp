...
class MainS extends CI_Controller{
	...
	public function getView($md, $md2 = 1, $md3=0, $md4=0, $fnd="0", $page=0){
		...
		$pagePerNum = PAGELINE;
		$config['base_url'] = '/statis/mainS/getView/'.$md.'/'.$md2."/".$md3."/".$md4."/".$fnd;
		...
		$param = array("mode" => "main", "md" => $md, "md2"=>$md2, "seMenu"=>getSeMenu($this->session->userdata('mainMenu')));
		...
		case 1:
			...
			case 1:
				...
				//스트리밍 사용통계
				$mechulSt = $this->statisdb->mechulAnz("st");
				//다운로드 사용통계
				$mechulDown = $this->statisdb->mechulAnz("down");
				
				//이용권 구매통계
				$gumeAnz = $this->statisdb->gumeAnz(1);
				
				//가입자 통계
				$memberAnz = $this->statisdb->memberAnz();
				
				
			
				//주문내역을 가져온다.
				//$query = $this->statisdb->statisAll();
				
				$this->load->view('statis/mrsaleAll', array_merge($plink, $param, array("mechulSt"=>$mechulSt, "mechulDown"=>$mechulDown, "cuponMechul"=>$gumeAnz, "memberAnz"=>$memberAnz)));
			break;
			}
		...
		case 2:
			...
			case 1:
				...
				//공지 리스트 
				$su = count($this->statisdb->statisTotalCount(1));
				...
				$this->load->view('statis/mrsaleAnz', array_merge($plink, $param, array("allbd"=>$query, "totalCount"=>$su, "seMenu"=>getSeMenu($this->session->userdata('mainMenu')))));
			break;
			}
		...
		//$this->load->view('member/allmem', array("allmem"=>$query->result(), $param));
		if(!$md2) $md2 = 0;
		if(!$md3) $md3 = 0;
		if(!$md4) $md4 = 0;
