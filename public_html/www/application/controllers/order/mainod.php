...
class Mainod extends CI_Controller{
	...
	public function getView($md, $md2 = 1, $md3=0, $md4=0, $fnd="0", $page=0){
		...
		$pagePerNum = PAGELINE;
		$config['base_url'] = '/order/mainO/getView/'.$md.'/'.$md2."/".$md3."/".$md4."/".$fnd;
		...
		case 1:
			...
			case 1:
				...
				//공지 리스트 
				$su = count($this->orderdb->orderTotalCount(1));
				$config["total_rows"] = $su;
				$this->pagination->initialize($config);
							
			
				//주문내역을 가져온다.
				$query = $this->orderdb->orderAll();
				
				$this->load->view('order/orderall', array_merge($plink, $param, array("allbd"=>$query, "totalCount"=>$su, "seMenu"=>getSeMenu($this->session->userdata('mainMenu')))));
			break;
			}
		...
		case 2:   //가입회원
			//회원리스트를 가져온다.
			$query = $this->member->memListAll("stat = 2");	
		break;
		case 3:   //가입회원
			//회원리스트를 가져온다.
			$query = $this->member->memListAll("stat = 4");
		break;
		}
