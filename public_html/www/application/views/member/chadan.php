...
?>
      ...
      <div class="tab">
        <ul>
          <li class="on"><a href="#">차단회원관리</a></li>
        </ul>
      </div>
      <!-- //tab -->
      ...
      <div class="srch">
             ...
             <select id="memgubun" name="memgubun" onchange="meminf.chnSelect(this, 3)">
      <?  
...
if($su < 1){
?>	
	<tr><td colspan="9">차단된 회원데이터가 없습니다.</td></tr>
...
?>   
      <tr>
        <td><?=($su - $num++)?></td>
        <td><a href="#" id="<?="ch-".$rows->id?>" class="desangList"><?=$rows->memid?></a></td>
        <td><?=$rows->name?></td>
		...
		?>
        	본인
        <? }else{ ?>
        <a href="#" onclick="meminf.delMem(<?=$rows->id?>, 'del', <?=$rows->potion?>, 0)" class="btn"><span>삭제</span></a>
