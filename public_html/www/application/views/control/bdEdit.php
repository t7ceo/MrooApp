...
?>
      ...
      <div class="tab">
        <ul>
          <li class="on"><a href="#">게시판 수정</a></li>
        </ul>
      </div>
      <!-- //tab -->
  ...
  <div id="join_mem" class="join_mem finder_div2" style="position:relative; margin:25px auto 0; height:350px; border:none; display:block;">
    				...
    				<form id="frmBd" name="frmBd" action="<?=$baseUrl?>control/edtBorder" method="POST">
                    <input type="hidden" name= "ci_t" value="<?=$cihs?>">
                    <input type="hidden" name= "rid" value="<?=$rows->id?>">
        ...
        <tr><th><span>게시판 이름</span></th><td>
            <input type="text" id="bdtit" name="bdtit" maxlength="10" style="width:70%;" value="<?=$rows->bdtit?>" />
        </td></tr>
...
<?
	...
	$bb = array('','','','');
	$bb[$rows->cogubunWR] = "selected";
?>
        
        <tr><th><span>게시판 노출설정</span></th><td>
<select id="cogubunWR" name="cogubunWR"><option value="0">선택</option><option value="1" <?=$bb[0]?>>전체 ( 본사+조합+계열사 )</option><option value="3" <?=$bb[3]?>>본사</option></select>
		...
		<p style="text-align:center; padding:25px 0;"><a href="#" class="btn_org" onclick="bdONEdit()"><span>게시판 수정</span></a>
